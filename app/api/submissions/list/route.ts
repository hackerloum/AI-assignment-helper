...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!user) {
      ...
      if (authError) {
        console.error('[List API] Auth error:', authError.message);
      }
    }
    
    if (sessionError) {
      console.error('[List API] Session error:', sessionError.message);
    }

    if (!user) {
      console.error('[List API] No user found. Session:', !!session, 'Session error:', sessionError?.message);
